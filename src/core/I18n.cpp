...
const QHash<QString, QString> kEnglish = {
    ...
    { QStringLiteral("仓库存储目录"), QStringLiteral("Repository storage folder") },
    { QStringLiteral("新增仓库时工作副本默认存放的目录；已有仓库不受影响"),
      QStringLiteral("Default folder for new working copies; existing repositories are unaffected") },
    { QStringLiteral("选择仓库存储目录"), QStringLiteral("Choose repository storage folder") },
